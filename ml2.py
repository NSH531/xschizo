...
def cop(d:list):
    ...
    for i in d:
        ...
        for file in glob.glob("f'/kaggle/input/xschizo115/ds000115_R2.0.0/{p}/*.nii"):
            s.append(file)
        print(s)
        #shutil.copy()
        con=i["cond"]
...
for dirname, _, filenames in os.walk('/kaggle/input/xschizo115/ds000115_R2.0.0'):
    for filename in filenames:
        ...
        if(filename=="participants.tsv"):
            ...
            p=df.to_dict()["participant_id"]
            cond=df.to_dict()["condit"]
            data1=[]
            scz=[]
            con=[]
            for i in range(len(p)):
                if("SCZ" in cond[i]):
                    scz.append(dict(p=p[i],cond=cond[i]))
                elif ("CON" in cond[i]):
                    con.append(dict(p=p[i],cond=cond[i]))
                data1.append(dict(p=p[i],cond=cond[i]))
            print(pd.DataFrame(scz))
            print(pd.DataFrame(con))
            cop(scz)
            #print(data1)
           # print(df.to_string()) 
